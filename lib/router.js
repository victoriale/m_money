...
Router.map(function(){
  // Temporary route definitions. These are made to remove warnings in console. (Other pages have links to these routes. Without the routes defined, a warning is thrown)
  // Delete this when creating actual route
  this.route('content.finance.directory');
  this.route('content.finance.listoflists');
  this.route('content.finance.contactus');
  this.route('content.finance.disclaimer');
  this.route('content.finance.listview');
  //End Temporary route definitions
...
/********************PROFILES********************/
 ...
 this.route('content.locationprofile',{
   path:'/:partner_id?/location/:state_id/:city_id?',
...
LocationProfileController = BaseController.extend({
 onBeforeAction: function(){
   ...
   console.log("STATE:", this.params.state_id);
   console.log("CITY:", this.params.city_id);
   Session.set("IsLocation", true);
   Session.set("state_id", this.params.state_id);
   ...
   if ( typeof Session.get('IsFirstRun') == "undefined" ) {
     ...
     ******************************************/
     //BATCH 1 call sending only first few modules data
     Meteor.call("GetProfileData", "location_profile", "batch_1", Session.get('state_id'), Session.get('city_id'), function(error, data){
       console.log("batch1", data);
       if(error || typeof data == 'undefined'){
         console.log('Invalid Exec Batch 1 Error',error);
         Session.set('IsError',true);
