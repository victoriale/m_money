...
Router.map(function(){
 ...
 /*********LIST PAGES**************/
  this.route('content.toplist',{
    path: '/:partner_id?/:loc_id?/top-list/:list_id',
  ...
  this.route('content.finance.directory',{
    path: '/:partner_id?/directory/profiles/page/:pageNum',
...
DirectoryController = BaseController.extend({
  onBeforeAction: function(){
    if( typeof Session.get('IsFirstRun') === 'undefined'){
      ...
      Meteor.call('GetDirectoryData', this.params.pageNum, query, function(error, data){
        if(error || typeof data === 'undefined'){
