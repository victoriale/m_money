...
Router.map(function(){
  ...
  this.route('partner.locationprofile',{
    path:'/:partner_id/:loc_id/:city?/location/:city_id?',
    controller: 'LocationProfileController'
  });
...
BaseController = RouteController.extend({
  before: function(){
    ...
    */
    Session.set('IsSubDomain', false);
    Session.set('isPartner', false);
    ...
    if ( typeof this.params.partner_id != "undefined" && typeof Session.get('IsGlobalFirstRun') == "undefined" ) {
      ...
      // Call domain API to get root domains for all verticals
      Meteor.call('GetPartnerDomain', this.params.partner_id, function(error, data) {
        if (error) {
          console.log('Partner Domain Error:', error);
          return false;
        }
        try {
          var domainData = JSON.parse(data.content);
          Session.set('isPartner', true);
          Session.set('domainData', domainData);
        } catch (e) {
          console.log('Partner Domain Error (parsing)', e);
        }
      });
