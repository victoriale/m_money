...
/********************PROFILES********************/
 ...
 this.route('content.executiveprofile',{
   ...
   //path:'/:partner_id?/executive/:compnay_id/:exec_name/:exec_id',
...
BaseController = RouteController.extend({
  onBeforeAction: function(){
    ...
    scrollUp();
    console.log("BASE CHECK:", Session.get('IsError'));
   ...
   if ( typeof Session.get('IsFirstRun') == "undefined" ) {
     ...
     ******************************************/
     ...
     Meteor.call("GetCompanyData",this.params.company_id, "batch_1", function(error, data){
       ...
       //test if :comp_name matches company with the data received if not set error as true
       /*
       if(typeof Session.get('pageCompany') == 'undefined'){
         Session.set('IsError',true);
         Session.set("IsData",false);
       }else{
       ...
       console.log("object",comp_check);
       console.log("object",comp_hyphen_check);
       console.log("object",exch);
   ...
   if ( typeof Session.get('IsFirstRun') == "undefined" ) {
     ...
     ******************************************/
     ...
     Meteor.call("GetExecData",Session.get('exec_id'), "batch_1", function(error, data){
       console.log("batch1", data);
       if(error){
         console.log('Invalid Exec Error',error);
         Session.set('IsError',true);
         return '';
       }else{
         for ( var module_name in data ) {
...
CollegeRivalsController = BaseController.extend({
 onBeforeAction: function(){
   ...
   if ( typeof Session.get('IsFirstRun') == "undefined" ) {
     ...
     Meteor method call for server side ajax
     path: '/:partner_id?/collegerivals/:exec_id',
     methods.js: WebpageData: function(exec_id, option)
     ******************************************/
     Meteor.call("WebpageData", this.params.exec_id, 'college_rivals', function(error, data){
       if(error){
         console.log('Invalid Team Error',error);
         Session.set('IsError',true);
         return '';
...
WorkHistoryController = BaseController.extend({
 onBeforeAction: function(){
   ...
   if ( typeof Session.get('IsFirstRun') == "undefined" ) {
     ...
     methods.js: WebpageData: function(exec_id, option)
     ******************************************/
     Meteor.call("WebpageData", this.params.exec_id, 'work_history', function(error, data){
       if(error){
         console.log('Invalid Team Error',error);
         Session.set('IsError',true);
         return '';
...
EducationController = BaseController.extend({
 onBeforeAction: function(){
   ...
   if ( typeof Session.get('IsFirstRun') == "undefined" ) {
     ...
     methods.js: WebpageData: function(exec_id, option)
     ******************************************/
     Meteor.call("WebpageData", this.params.exec_id, 'education', function(error, data){
       if(error){
         console.log('Invalid Team Error',error);
         Session.set('IsError',true);
         return '';
...
FinancialBaseController = BaseController.extend({
  onBeforeAction: function(){
    ...
    Meteor.call("EarningsIndie", this.params.company_id, function(error, data){
      if(error){
        console.log('Invalid Team Error',error);
        Session.set('IsError',true);
        return '';
...
MoneyMemoryController = FinancialBaseController.extend({
 onBeforeAction: function(){
   console.log("PARTNER",this.params.partner_id);
   console.log("RENDERING Money Memory");
   Session.set('IsData',true);
...
EarningsController = FinancialBaseController.extend({
  onBeforeAction: function() {
    if ( typeof Session.get("IsFirstRun") == "undefined" ) {
      Session.set("IsFirstRun",false);
...
FinancialOverviewController = FinancialBaseController.extend({
 onBeforeAction: function(){
   console.log("PARTNER",this.params.partner_id);
...
CompetitorController = FinancialBaseController.extend({
 onBeforeAction: function(){
   console.log("PARTNER",this.params.partner_id);
