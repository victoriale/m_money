...
/*********STANDALONE PAGES**************/
  ...
  this.route('content.moneymemory',{
   path: '/:partner_id?/money-memory/:comp_id',
...
CompensationController = BaseController.extend({
 onBeforeAction: function(){
   ...
   if ( typeof Session.get('IsFirstRun') == "undefined" ) {
     ...
     methods.js: WebpageData: function(exec_id, option)
     ******************************************/
     Meteor.call("WebpageData", this.params.exec_id, 'compensation', function(error, data){
       if(error){
         console.log('Invalid Team Error',error);
         Session.set('IsError',true);
         return '';
...
FinancialBaseController = BaseController.extend({
  onBeforeAction: function(){
    ...
    Meteor.call("WebpageData", this.params.exec_id,  function(error, data){
      if(error || data.success == false){
        console.log('Invalid Team Error',error);
        Session.set('IsError',true);
        return '';
...
MoneyMemoryController = BaseController.extend({
 onBeforeAction: function(){
   ...
   Session.set('IsData',true);
   //WebpageData: function(exec_id, option)
   Meteor.call("WebpageData", this.params.comp_id, 'money_memory', function(error, data){
     if(error || data.success == false){
       console.log('Invalid Team Error',error);
       Session.set('IsError',true);
       return '';
