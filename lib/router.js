...
// Router.route('/')
cdnURL = undefined;
var apiUrl = "http://dev-finance-api.synapsys.us:280/call_controller.php";
       ...
       if(error || typeof data == 'undefined'){
         ...
         Meteor.call("GetLocationData", Session.get('loc_id'), "batch_2", function(error, data){
          ...
          //  GetLocationData: function(loc_id, batchNum, graph_option) send true to graph_option
           Meteor.call("GetLocationData", Session.get('loc_id'), "batch_2", true, function(error, data){
             //console.log("batch2", data);
             if(error){
               console.log('Invalid Exec Batch 2 Error',error);
               return '';
             }
             console.log('5 year data received',data);
             var composite_history = Session.get('location_daily_update');
             if(typeof composite_history['composite_history'] != 'undefined' || composite_history['composite_history'] != null){
               console.log('appending to',composite_history['composite_history']);
             }
           });//end BATCH 2
         });//end BATCH 2
