...
PHcheck = function(data){
  ...
  if(typeof check == 'undefined' || check == ''){
    ...
    if(check.Salary == '0' || typeof check.Salary == 'undefined' || check.Salary == Infinity){
      data.o_compensation.Salary = 0;
    }else{
      data.o_compensation.Salary = Number(data.o_compensation.Salary);
    }

    if(check.Bonus == '0' || typeof check.Bonus == 'undefined' || check.Bonus == Infinity){
      data.o_compensation.Bonus = 0;
    }else{
      data.o_compensation.Bonus = Number(data.o_compensation.Bonus);
    }

    if(check.TotalST == '0' || typeof check.TotalST == 'undefined' || check.TotalST == Infinity){
      data.o_compensation.TotalST = 0;
    }else{
      data.o_compensation.TotalST = Number(data.o_compensation.TotalST);
    }

    if(check.RestrictedStock == '0' || typeof check.RestrictedStock == 'undefined' || check.RestrictedStock == Infinity){
      data.o_compensation.RestrictedStock = 0;
    }else{
      data.o_compensation.RestrictedStock = Number(data.o_compensation.RestrictedStock);
    }

    if(check.AllOtherLT == '0' || typeof check.AllOtherLT == 'undefined' || check.AllOtherLT == Infinity){
    ...
    if(check.TotalComp == '0' || typeof check.TotalComp == 'undefined' || check.TotalComp == Infinity){
