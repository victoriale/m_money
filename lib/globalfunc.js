...
createGenericString = function (error, data) {
  ...
  */
    ...
    convertToUnix = function(input){
      if(typeof input == 'undefined' || input == null){ // if input is undefined, generate current day's unix
        input = moment(new Date()).tz('America/New_York').unix() * 1000;
        return input;
      }
      if(typeof input == 'string'){
        if(input.match(/^[0-9]+$/) != null){
          ...
          return moment(input).tz('America/New_York').unix() * 1000;
        }
      } else {
...
/*
  ...
  */
  ...
  globalDateFormat = function(value,identifier) {
    ...
    var newDate;
    var longDay = moment(unixValue).tz('America/New_York').format('dddd');
    var shortDay = moment(unixValue).tz('America/New_York').format('D');
    ...
    var shortDate = moment(unixValue).tz('America/New_York').format('MM/DD/YY');
    var year = moment(unixValue).tz('America/New_York').format('YYYY');
    ...
    switch(identifier) {
      ...
      case 'shortDate':
        ...
        return newDate;
      case 'timeZone':
        newDate = longDay + ', ' + month + ' ' + shortDay + ', ' + year + ' ' + timeZone; //Tuesday, Oct. 03, 2006 5:30:10PM (EST)
        return newDate;
      case 'dayOfWeek':
