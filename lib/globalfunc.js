...
/*
  Function formats date into any of the 4 standard time formats
  ...
  */
  ...
  globalDateFormat = function(value,identifier) {
    ...
    if(unixValue.toString().length <= 11){
      unixValue = Number(unixValue) * 1000; // formats the Unix correctly
    } else {
      unixValue = Number(unixValue); // assumes that if the unix value is greater than 11, it is 13(the correct unix).
    }
    var newDate;
